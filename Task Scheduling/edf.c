...
int main() {
    ...
    for (T = 0; T <= 200; T++) {
        ...
        if(cpuUtil > 1) {
            printf("\nUtilization greater than 1, unschedulable, exiting program\n");
            exit(-1);
        }
        else if(T == absDeadlineA && tA < execTimeA) {
            printf("\nDeadline A missed at T=%d, exiting program\n", T);
            exit(-1);
        }
        else if(T == absDeadlineB && tB < execTimeB) {
            printf("\nDeadline B missed at T=%d, exiting program\n", T);
            exit(-1);
        }
        // End of your code
        ...
        if (T % periodA == 0 && T % periodB == 0) {
            ...
            // EDF: First deadline = higher priority
        ...
        if (T % periodA == 0 && T % periodB != 0) {
            ...
            if (tB < execTimeB) { // process B is unfinished yet
                ...
                if(absDeadlineB < absDeadlineA) {
                    doA = 0;
                    doB = 1;
                    printf("when T=%d, program continued to run process B%d!\n", T, jB);
                }
                else {
                    doA = 1;
                    doB = 0;
        ...
        if (T % periodA != 0 && T % periodB == 0) {
            ...
            if (tA < execTimeA) { // process A is unfinished yet
                ...
                if(absDeadlineA < absDeadlineB) {
                    doA = 1;
                    doB = 0;
                    printf("when T=%d, program continued to run process A%d!\n", T, jA);
                }
                else {
                    doA = 0;
                    doB = 1;
